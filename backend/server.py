...
@api_router.get("/availability/{institution_id}/{program_id}/{date}")
...
@api_router.get("/calendar/{institution_id}/{year}/{month}")
async def get_calendar_availability(institution_id: str, year: int, month: int):
    ...
    if institution_id == "demo":
        ...
        for day in range(1, num_days + 1):
            ...
            # Mark some days as available (with dots in Figma design)
            has_availability = day % 3 != 0  # Simple pattern for demo
            dates.append({
                "date": date_str,
