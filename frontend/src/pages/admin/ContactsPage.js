...
const DELIVERABILITY = {
  delivered: { label: 'Doručitelný', className: 'bg-emerald-100 text-emerald-700', Icon: CheckCircle2 },
  bounced_soft: { label: 'Dočasný problém', className: 'bg-amber-100 text-amber-800', Icon: AlertTriangle },
  bounced_hard: { label: 'Neplatná adresa', className: 'bg-rose-100 text-rose-700', Icon: AlertCircle },
  complained: { label: 'Označeno jako spam', className: 'bg-rose-100 text-rose-700', Icon: AlertCircle },
  suppressed: { label: 'Blokováno', className: 'bg-rose-100 text-rose-700', Icon: AlertCircle },
  unsubscribed: { label: 'Odhlášeno', className: 'bg-slate-200 text-slate-700', Icon: ShieldX },
  failed: { label: 'Doručení selhalo', className: 'bg-rose-100 text-rose-700', Icon: AlertCircle },
  unknown: { label: 'Neověřeno', className: 'bg-slate-100 text-slate-600', Icon: Mail },
};

function DeliverabilityBadge({ status = 'unknown' }) {
  const item = DELIVERABILITY[status] || DELIVERABILITY.unknown;
  const Icon = item.Icon;
  return (
    <Badge className={`${item.className} hover:${item.className} gap-1 font-normal`}>
...
const ContactsPage = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (deliverabilityFilter !== 'all') params.set('deliverability', deliverabilityFilter);
  ...
  }, [typeFilter, sourceFilter, consentFilter, deliverabilityFilter, searchQuery]);
        ...
        <Card className="p-3 md:p-4">
          <div className="flex flex-col lg:flex-row gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Select value={deliverabilityFilter} onValueChange={setDeliverabilityFilter}>
                <SelectTrigger className="w-[190px]" data-testid="filter-deliverability">
                  <Mail className="w-3.5 h-3.5 mr-1.5" /><SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Všechny adresy</SelectItem>
                  <SelectItem value="problem">Neplatné / blokované</SelectItem>
                  <SelectItem value="warning">Dočasný problém</SelectItem>
                  <SelectItem value="ok">Doručitelné</SelectItem>
                  <SelectItem value="unknown">Neověřené</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
            ...
            <table className="w-full text-sm" data-testid="contacts-table">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">Doručitelnost</th>
              ...
              <tbody>
                ...
                {!loading && contacts.length === 0 && (
                  <tr><td colSpan={8} className="text-center py-12" data-testid="contacts-empty">
                ...
                {contacts.map(c => (
                  <tr key={c.id} onClick={() => setSelectedId(c.id)}
                    ...
                    <td className="px-4 py-3"><DeliverabilityBadge status={c.deliverability_status} /></td>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-full sm:max-w-md bg-white shadow-2xl z-50 overflow-y-auto" data-testid="contact-detail-panel">
        ...
        {contact && (
          <div className="p-5 space-y-6">
            {['bounced_hard', 'complained', 'suppressed', 'unsubscribed', 'failed'].includes(contact.deliverability_status) && (
              <section className="rounded-lg border border-rose-200 bg-rose-50 p-4" data-testid="invalid-contact-warning">
                <div className="flex gap-3">
                  <AlertTriangle className="w-5 h-5 text-rose-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <h3 className="font-semibold text-rose-900">E-mail se prokázal jako nedoručitelný</h3>
                    <p className="text-sm text-rose-800 mt-1">
                      Kontakt je vyřazen z dalších propagačních rozesílek.
                      {contact.deliverability_reason && <> Důvod: {contact.deliverability_reason}</>}
                    </p>
                    <Button
                      type="button"
                      variant="outline"
                      ...
                      data-testid="invalid-contact-delete-btn"
                    >
                      <Trash2 className="w-4 h-4 mr-2" /> Smazat kontakt
                    </Button>
                  </div>
                </div>
              </section>
            ...
            {contact.deliverability_status === 'bounced_soft' && (
              <section className="rounded-lg border border-amber-200 bg-amber-50 p-4">
                <p className="text-sm text-amber-900">
                  Doručení má dočasný problém. Kontakt zatím nemažte; systém počká na další výsledek doručení.
                </p>
              </section>
            )}
