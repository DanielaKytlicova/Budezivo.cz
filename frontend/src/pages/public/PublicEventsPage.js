...
export default function PublicEventsPage() {
  ...
  const fetchEventDetail = async (eventId) => {
    try {
      ...
      toast.error('Nepodařilo se načíst detail události');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Find email from form values or form fields
            ...
            {result.qr_payload && result.total_amount > 0 && (
              <div className="border-t pt-6 mt-6 space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4 text-left space-y-2 text-sm">
                  {result.payment_settings?.account_number && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono font-medium">{result.payment_settings.account_number}/{result.payment_settings.bank_code}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-gray-500">Částka:</span>
                    <span className="font-medium">{result.total_amount} Kč</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Variabilní symbol:</span>
                    <span className="font-mono font-medium">{result.variable_symbol}</span>
                  </div>
                  {result.payment_settings?.account_name && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">Příjemce:</span>
                      <span className="font-medium">{result.payment_settings.account_name}</span>
                    </div>
                  )}
                </div>
                ...
                <Button
                  ...
                  className="w-full"
                  onClick={() => {
                    const text = `Účet: ${result.payment_settings?.account_number}/${result.payment_settings?.bank_code}\nČástka: ${result.total_amount} Kč\nVS: ${result.variable_symbol}`;
                    navigator.clipboard.writeText(text);
                    toast.success('Platební údaje zkopírovány');
                  }}
      ...
      <div className="min-h-screen bg-[#F8F9FA] py-8 px-4">
        <div className="max-w-lg mx-auto">
          ...
          <Card className="p-6">
            ...
            {selectedDate && (
              <p className="text-sm text-gray-500 mb-4">Termín: {formatDate(selectedDate.start_datetime)}</p>
            )}
            {selectedEvent.price > 0 && (
              <p className="text-sm font-medium text-slate-700 mb-4">Cena: {selectedEvent.price} Kč</p>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {(selectedEvent.form_fields || []).map((field) => (
                <div key={field.id}>
                  ...
                  {field.type === 'number' && (
                    <Input type="number" value={formValues[field.id] || ''} onChange={e => setFormValues(p => ({ ...p, [field.id]: e.target.value }))} required={field.required} className="mt-1" data-testid={`field-${field.id}`} />
                  )}
                  {field.type === 'select' && (
                    <select value={formValues[field.id] || ''} onChange={e => setFormValues(p => ({ ...p, [field.id]: e.target.value }))} required={field.required} className="mt-1 w-full border rounded-md px-3 py-2 text-sm" data-testid={`field-${field.id}`}>
                  ...
                  {field.type === 'checkbox' && (
                    <label className="flex items-center gap-2 mt-1 cursor-pointer">
                      <input type="checkbox" checked={!!formValues[field.id]} onChange={e => setFormValues(p => ({ ...p, [field.id]: e.target.checked }))} className="rounded" data-testid={`field-${field.id}`} />
              ...
              <Button type="submit" disabled={submitting} className="w-full bg-[#5a7aae] hover:bg-[#4a6a9e] h-12" data-testid="submit-application-btn">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <Card className="overflow-hidden">
            <div className="p-6 md:p-8">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-4 border rounded-lg transition-all ${
                          isSelected ? 'border-[#5a7aae] bg-[#5a7aae]/5 ring-1 ring-[#5a7aae]'
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-right">
                            {isFull ? (
                              <span className="text-xs text-red-500 font-medium">Obsazeno</span>
                            ) : (
              ...
              <Button
                onClick={() => { setFormValues({}); setStep('form'); }}
                disabled={!selectedDate && (selectedEvent.dates || []).length > 0}
                className="w-full bg-[#5a7aae] hover:bg-[#4a6a9e] h-12"
